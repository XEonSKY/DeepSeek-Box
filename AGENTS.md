...
- agent 只做静态验证（`npm run check` = typecheck + lint + 单测），运行时行为由用户验证。

## 常用命令

| 命令 | 用途 |
|---|---|
| `npm run check` | typecheck + lint + 单元测试（提交前必过，pre-commit 已自动执行） |
| `npm run test` | 只跑单元测试（等价于 `check` 的最后一步） |
| `npm run test:watch` | 单元测试 watch 模式（开发时用） |
| `npm run test:coverage` | 带覆盖率报告（输出到 `.agents/temp/coverage`） |
