...
- 「设置 → 常规 → 网络」把 npm 镜像源切到 `npmmirror`；
- 下载支持断点续传，中断后重试会接着下；
- 安装过程可随时点「取消」，之后重试。

## 按 F12 控制台没反应

**开发模式默认隐藏**，得先解锁：打开「设置 → 关于」，**连点「应用版本号」5 次**（1.5 秒内连击），该页才会出现「开发模式」开关；把它打开后 `F12` 才会生效。默认关闭以避免误开。
...
::: tip 注意区分「多实例」与「多窗口」
本应用**支持多窗口**——在标签上右键「在新窗口打开」即可开出副窗口（见[界面与操作](/zh/user/usage)）。这是同一个应用内的多个窗口。
:::
