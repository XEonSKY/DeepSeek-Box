 ...
 * 扩展包管理器（内置扩展）。
 *
 * 外部扩展除了「文件夹」形态，还支持压缩包形态（`.zip` 与 `.xeonsky-ext`——后者是
 * 本项目的 zip 变种，内容相同）。解压这件事不放进加载器：加载器不做格式解析，
 * 它只认「目录里有 manifest.json」。真正的解压能力由本扩展提供 —— 内部转发给
 * `xeonsky.zip`（7-Zip 命令行核心），于是 zip/xeonsky-ext 的支持随 7-Zip 的格式表走。
 *
