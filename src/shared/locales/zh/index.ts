...
export default {
    ...
    dshMissing: {
        ...
        executing: '执行中…',
        registry: 'npm 镜像源',
        registryHint: '本次安装与后续 DeepSeek Harness 更新都会使用所选镜像源拉取。',
        configDir: '配置目录',
        choose: '选择…',
        restoreDefault: '恢复默认',
        ...
        installSettingsHint: '镜像源、配置目录与代理；改完立即生效于本次安装。',
    ...
    sv: {
        ...
        plugins: {
            pnpmTitle: '内置 pnpm',
            pnpmHint: 'dsh 安装插件时会把参数转发给 pnpm；这里像 npm 一样由应用下载并代管，首次使用时自动就绪。',
            pnpmNotDownloaded: '尚未下载',
            pnpmDownload: '下载 pnpm',
            pnpmUpdate: '更新 pnpm',
            pnpmOk: 'pnpm {version} 已就绪。',
            pnpmFail: 'pnpm 下载失败。',
            pnpmListHint: '列表来自当前 registry（新 → 旧）；安装作用于配置目录里的内置 pnpm。',
        ...
        models: {
            ...
            consentDesc:
                'DeepSeek Box 需要读取 dsh 的本地配置与凭据文件，以便列出各供应商提供的模型，并用已保存的密钥向供应商接口查询余额。页面不展示任何密钥；查询只在本机发起，结果不写入日志，也不会发送到别处。',
            consentPathValue: '~/.dsh/settings.yaml 与 ~/.dsh/.credentials.yaml（或 $DSH_HOME 指向的目录）',
            consentAgree: '同意并读取',
            consentDeny: '暂不',
            ...
            clickRefresh: '点击刷新余额',
            errSettingsMissing: '未找到 dsh 配置（settings.yaml）。请先启动一次 DeepSeek Harness。',
            errSettingsParse: 'dsh 配置无法解析（settings.yaml）。请修复后重试。',
    ...
    m: {
        dsh: {
            ...
            bundledPnpmMissing: '内置 pnpm 解压后缺少入口文件（pnpm.cjs）。',
